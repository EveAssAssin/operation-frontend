...
function SubjectsModal({ subjects: initSubjects, onClose, onSaved }) {
  ...
  // 切換隱藏／顯示
  async function handleToggleActive(id, makeHidden) {
    try {
      await checksApi.updateSubject(id, { is_active: !makeHidden });
      setList(prev => prev.map(s => s.id === id ? { ...s, is_active: !makeHidden } : s));
      onSaved();
    } catch(e) { alert(e.message || '操作失敗'); }
  }
    ...
    return (
      <div key={s.id} style={{
        ...
        {editingId === s.id ? (
          ...
          <>
            <span style={{ flex: 1, fontSize: indent ? 13 : 14, color: C.textDark, fontWeight: indent ? 400 : 600, textDecoration: isHidden ? 'line-through' : 'none' }}>{s.name}{isHidden && ' (已隱藏)'}</span>
            {/* 母分類選擇器 */}
            <select
              value={s.parent_id || ''}
            ...
            <button onClick={() => handleToggleActive(s.id, !isHidden)}
              title={isHidden ? '顯示（解除隱藏）' : '隱藏（不再出現在下拉選單）'}
...
function MergeSubjectsModal({ subjects, onClose, onSaved }) {
  ...
  // 排序：母分類在前、子科目接在自己母分類後面
  const sortedSubjects = (() => {
    const categories = subjects.filter(s => !s.parent_id);
    const childrenOf = (id) => subjects.filter(s => s.parent_id === id);
    const orphans = subjects.filter(s => s.parent_id && !subjects.find(c => c.id === s.parent_id));
    const out = [];
    for (const c of categories) {
      out.push({ ...c, _level: 0 });
      for (const ch of childrenOf(c.id)) out.push({ ...ch, _level: 1 });
    }
    for (const o of orphans) out.push({ ...o, _level: 0 });
    return out;
  })();
    ...
    <Modal title="🔀 合併科目" onClose={onClose} width={500}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{
          ...
          {sortedSubjects.filter(s => s.id !== keepId).map(s => (
            <label key={s.id} style={{
              display: 'flex', alignItems: 'center', gap: 10,
              padding: s._level > 0 ? '8px 14px 8px 36px' : '10px 14px',
              borderBottom: `1px solid ${C.border}`,
              cursor: 'pointer',
              background: mergeIds.includes(s.id) ? '#fff3cd' : (s._level > 0 ? '#fafaf8' : 'transparent'),
            }}>
              <input
                type="checkbox"
              ...
              <span style={{ fontSize: s._level > 0 ? 13 : 14, color: C.textDark, fontWeight: s._level > 0 ? 400 : 600 }}>{s.name}</span>
            </label>
          ))}
          {subjects.length <= 1 && (
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          >
            ...
            {subjectList.filter(s => s.is_active !== false || s.id === form.subject_id).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          {addingSubject && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div>
            ...
            >
              ...
              {subjectsTree.filter(cat => cat.is_active !== false).map(cat => (
          ...
          <div>
            ...
            >
              ...
              {selectedCategory && (
                (subjectsTree.find(c => String(c.id) === String(selectedCategory))?.children || [])
...
function Loading() {
  return (
        <div style={{ textAlign: 'center', padding: 40, color: '#9a8878' }}>載入中...</div>
  );
}
...
function EmptyBox({ text }) {
  return (
    <div style={{
      textAlign: 'center', padding: '48px 20px', color: '#9a8878',
      background: '#fff', borderRadius: 12, border: '1px solid #e0d5c8',
    }}>
      {text}
    </div>
  );
...
const inputStyle = {
  ...
  border: '1px solid #e0d5c8', borderRadius: 6,
